...
router.post('/confirmfriend', async (req, res) => {
       ...
       try {
              ...
              result1.friends.push({ friend_id: req.body.sender_id });
              await result1.save();
              ...
              result2.friends.push({ friend_id: req.body.reciever_id });
              await result2.save();
...
router.post('/pendingfriend', async (req, res) => {
       try {
              ...
              result1.pending_friends_sender.push({ friend_id: req.body.reciver_id });
              await result1.save();
              let result2 = await Person.findOne({ userId: req.body.reciver_id });
              result2.pending_friends_reciever.push({ friend_id: req.body.sender_id });
              await result2.save();
              res.status(200).json("success");
       }
       catch (error) {
